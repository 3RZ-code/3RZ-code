<h1 align="center">Hi, I'm Michał 👋</h1>

<p align="center">
  <strong>Python & Django Developer</strong> · AI / Data Science Enthusiast · Computer Science Student
</p>

<p align="center">
  I build practical backend applications, work with data, explore algorithms  
  and turn ideas into clean, usable software.
</p>

<p align="center">
  <a href="https://www.linkedin.com/in/michal-kakala">
    <img src="https://img.shields.io/badge/LinkedIn-111827?style=for-the-badge&logo=linkedin&logoColor=0A66C2" />
  </a>
  <a href="https://www.kaggle.com/michakakaa">
    <img src="https://img.shields.io/badge/Kaggle-111827?style=for-the-badge&logo=kaggle&logoColor=20BEFF" />
  </a>
  <a href="https://zindi.africa/users/3RZ">
    <img src="https://img.shields.io/badge/Zindi-111827?style=for-the-badge" />
  </a>
</p>

---

## About me

- 🎓 Computer Science student
- 🐍 Focused on **Python**, **Django** and **SQL**
- 🧠 Interested in **AI**, **machine learning** and **data science**
- ⚙️ I enjoy backend development, algorithms and problem solving
---

## Tech stack

### Languages & Backend

<p>
  <img src="https://img.shields.io/badge/Python-111827?style=flat-square&logo=python" />
  <img src="https://img.shields.io/badge/Django-111827?style=flat-square&logo=django" />
  <img src="https://img.shields.io/badge/Java-111827?style=flat-square&logo=openjdk" />
  <img src="https://img.shields.io/badge/C++-111827?style=flat-square&logo=cplusplus" />
</p>

### Databases

<p>
  <img src="https://img.shields.io/badge/PostgreSQL-111827?style=flat-square&logo=postgresql" />
  <img src="https://img.shields.io/badge/SQLite-111827?style=flat-square&logo=sqlite&logoColor=white" />
  <img src="https://img.shields.io/badge/SQL-111827?style=flat-square&logo=postgresql" />
</p>

### Data Science & AI

<p>
  <img src="https://img.shields.io/badge/Pandas-111827?style=flat-square&logo=pandas" />
  <img src="https://img.shields.io/badge/NumPy-111827?style=flat-square&logo=numpy" />
  <img src="https://img.shields.io/badge/scikit--learn-111827?style=flat-square&logo=scikitlearn" />
  <img src="https://img.shields.io/badge/Jupyter-111827?style=flat-square&logo=jupyter" />
  <img src="https://img.shields.io/badge/Machine%20Learning-111827?style=flat-square" />
</p>

### Frontend & Tools

<p>
  <img src="https://img.shields.io/badge/HTML5-111827?style=flat-square&logo=html5" />
  <img src="https://img.shields.io/badge/CSS3-111827?style=flat-square&logo=css3" />
  <img src="https://img.shields.io/badge/JavaScript-111827?style=flat-square&logo=javascript" />
  <img src="https://img.shields.io/badge/React-111827?style=flat-square&logo=react&logoColor=61DAFB" />
</p>

<p>
  <img src="https://img.shields.io/badge/Git-111827?style=flat-square&logo=git" />
  <img src="https://img.shields.io/badge/GitHub-111827?style=flat-square&logo=github" />
  <img src="https://img.shields.io/badge/Linux-111827?style=flat-square&logo=linux" />
  <img src="https://img.shields.io/badge/Arch%20Linux-111827?style=flat-square&logo=archlinux" />
  <img src="https://img.shields.io/badge/Docker-111827?style=flat-square&logo=docker" />
  <img src="https://img.shields.io/badge/Power%20BI-111827?style=flat-square&logo=powerbi&logoColor=F2C811" />
</p>

---

## Featured projects

### BKIHACK / KOŚCIUSZKON 2026

Django-based project combining backend development, AI features and text-to-speech functionality.

**Tech:** Django, Python, AI, TTS

---

### 8-puzzle algorithms

Implementation and analysis of classic search algorithms for solving the 8-puzzle problem.

**Tech:** Python, BFS, DFS, A*, data analysis

---

### Wine classifier

Machine learning project focused on classification, data preprocessing and model evaluation.

**Tech:** Python, Pandas, scikit-learn, machine learning

---

### Database & backend projects

Projects involving relational databases, SQL queries, triggers, procedures and application logic.

**Tech:** SQL, PostgreSQL, backend development


## GitHub stats

<p align="center">
  <img height="165" src="https://github-readme-stats.vercel.app/api/top-langs/?username=3RZ-code&layout=compact&theme=tokyonight&hide_border=true" />
</p>
